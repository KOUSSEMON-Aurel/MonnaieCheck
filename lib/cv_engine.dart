...
class CvEngine {
  ...
  (cv.Mat, bool) detectArt14Ink(cv.Mat billetMat) {
    ...
    final cond1 =
        cv.compare(deltaL, cv.multiply(chroma, chroma, scale: 3.0), cv.CMP_LT);

    // 6. Chroma threshold (> 8) to ignore digital noise
    final thresh8 = cv.Mat.fromScalar(
        chroma.rows, chroma.cols, chroma.type, cv.Scalar(8, 0, 0, 0));
    ...
    final count = cv.connectedComponentsWithStats(
        mask, labels, stats, centroids, 8, cv.MatType.CV_32S, cv.CCL_DEFAULT);
  ...
  String identifyDenomination(cv.Mat alignedBillet) {
    ...
    ranges.forEach((deno, hRange) {
      cv.Mat mask;
      final lower = cv.Mat.fromScalar(hsv.rows, hsv.cols, hsv.type,
          cv.Scalar(hRange[0].toDouble(), 50, 50, 0));
      final upper = cv.Mat.fromScalar(hsv.rows, hsv.cols, hsv.type,
          cv.Scalar(hRange[1].toDouble(), 255, 255, 0));

      if (deno == '500') {
        final mask1 = cv.inRange(hsv, lower, upper);
        final lower2 = cv.Mat.fromScalar(hsv.rows, hsv.cols, hsv.type,
            cv.Scalar(hRange[2].toDouble(), 50, 50, 0));
        final upper2 = cv.Mat.fromScalar(hsv.rows, hsv.cols, hsv.type,
            cv.Scalar(hRange[3].toDouble(), 255, 255, 0));
  ...
  double calculateConvexity(cv.Mat coinMat) {
    ...
    final gray = coinMat.type == cv.MatType.CV_8UC1
        ? coinMat
        : cv.cvtColor(coinMat, cv.COLOR_BGR2GRAY);

    final blur = cv.gaussianBlur(gray, (5, 5), 0);
    final thresh =
        cv.threshold(blur, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU).$2;
